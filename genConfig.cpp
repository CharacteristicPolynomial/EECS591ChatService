...
int main (int argc, const char * argv[]) { // arguments: (number of replicas) 
    // Parse args
    if (argc <2) {
        cerr << "Error: Missing arguments. (number of replicas)" << endl;
        return -1;
    }
    ...
    ofstream ofs;
    ofs.open(get_config(ADDRESS_FILE), ios_base::out | ios_base::trunc);
    if(ofs.fail()) {
